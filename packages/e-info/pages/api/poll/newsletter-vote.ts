...
const CREATE_NEWSLETTER_POLL_RESULT_WITH_MEMBER = `
  mutation CreateNewsletterPollResultWithMember(
    $pollId: ID!
    $memberId: ID!
    $result: Int!
  ) {
...
const CREATE_NEWSLETTER_POLL_RESULT_ANONYMOUS = `
  mutation CreateNewsletterPollResultAnonymous($pollId: ID!, $result: Int!) {
    createPollResult(
      data: {
        poll: { connect: { id: $pollId } }
        result: $result
...
export default async function handler(
  ...
  try {
    const { pollId, result, memberId, firebaseId, idToken, turnstileToken } =
      req.body as RequestBody

    // Validate required fields
    if (!pollId || result === undefined) {
    ...
    if (isMemberVote) {
      ...
      }>(CREATE_NEWSLETTER_POLL_RESULT_WITH_MEMBER, {
        pollId,
        memberId,
        result,
      })
      ...
      if (mutationResult.error) {
        console.error(
          '[API /poll/newsletter-vote] GraphQL error:',
      ...
      const mutationResult = await serverGraphQL<{
        createPollResult: { id: string; result: number }
      }>(CREATE_NEWSLETTER_POLL_RESULT_ANONYMOUS, {
        pollId,
        result,
      })
    ...
    console.error('[API /poll/newsletter-vote] Error:', error)
